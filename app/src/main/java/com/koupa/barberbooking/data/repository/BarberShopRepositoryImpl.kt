...
class BarberShopRepositoryImpl @Inject constructor() : BarberShopRepository {
    ...
    override suspend fun createShop(
        ...
        return try {
            ...
            // Get Supabase user ID from phone
            val userResult = supabase.from("users")
                .select {
                    filter { eq("phone_number", firebaseUser.phoneNumber ?: "") }
                }
                .decodeList<JsonObject>()
                .firstOrNull()
                ?: return Result.failure(Exception("User not found"))

            val userId = userResult["id"]?.jsonPrimitive?.content
                ?: return Result.failure(Exception("User not found"))

            val insertData = BarberShopMapper.toInsertMap(
                ownerId = userId,
                shopName = shopName,
                city = city,
    ...
    override suspend fun updateShop(shop: BarberShop): Result<BarberShop> {
        return try {
            val updates = buildJsonObject {
                ...
                put("city", shop.city)
                put("wilaya_code", shop.wilayaCode)
                shop.latitude?.let { put("latitude", it) }
                shop.longitude?.let { put("longitude", it) }
            }
